...
class flashManagement extends React.Component {
    constructor(props) {
        super(props);
        let topMenuCode = '';
        if (sessionStorage.getItem('flashMenuName') && window.location.pathname !== '/flashManagement') {
            topMenuCode = sessionStorage.getItem('flashMenuName');
        } else {
            topMenuCode = 'home';
        }
        this.state = {
            ...
            // 菜单对应跳转url
            top2SubObj: {
                'home': '/flashManagement',
                'transactionPairs': '/flashManagement/transactionPairs',
                'record': '/flashManagement/record',
                'quotation': '/flashManagement/quotation'
            }
        };
    }
    ...
    handleTopMenuClick = ({key}) => {
        if (key && key !== 'user') {
            ...
            sessionStorage.setItem('flashMenuName', key);
