...
class BonusPool extends React.Component {
    ...
    doAdjust = (adjustCount) => {
        ...
        Promise.all([
            ...
            fetch(610660, {
                adjustCount,
                poolId: moneyParse(Number(this.state.selectedData.id), '', this.state.selectedData.symbol)
            })
        ]).then(() => {
            this.setState({
                adjustPopupShow: false,
            ...
            <div className="superNodeBonusPool-wrapper">
                <div className="superNodeBonusPool-section" style={{marginBottom: '30px'}}>
                    <div className="superNodeBonusPool-property">
                        <samp>总资产折合</samp>
                        <p>{formatMoney(String(totalAmountData.totalUsdtPrice).split('.')[0], '8', '100000000')} USDT<i>={totalAmountData.totalCnyPrice}CNY</i></p>
                    </div>
