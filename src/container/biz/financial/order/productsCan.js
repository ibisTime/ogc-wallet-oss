...
import {
  ...
  cancelFetching,
  setSearchData
} from '@redux/biz/financial/productsCan';
...
class ProductsCan extends React.Component {
  render() {
    const fields = [{
      title: '名称（中文简体）',
      field: 'nameZhCn',
      ...
      btnEvent: {
        ...
        investFlowAll: (selectedRowKeys, selectedRows) => {
          if (!selectedRowKeys.length) {
            ...
            this.props.history.push(`/bizFinancial/productsRaise/investFlowAll?code=${selectedRows[0].code}&menu=can`);
          }
        },
        repaymentPlan: (selectedRowKeys, selectedRows) => {
          if (!selectedRowKeys.length) {
            ...
            this.props.history.push(`/bizFinancial/repaymentPlan?type=khkuan&productCode=${selectedRows[0].code}&symbol=${selectedRows[0].symbol}&status=${selectedRows[0].status}`);
          }
        }
      }
    });
